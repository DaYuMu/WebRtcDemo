...
public class PeerSignalConnectionHelper {


    public final static String TAG = "dds_webRtcSignalHelper";
    ...
    public Map<String, PeerSignalConnectionHelper.Peer> _connectionPeerDic;
    ...
    private PeerSignalConnectionHelper.Role _role;
    ...
    public PeerSignalConnectionHelper(IWebSocket webSocket, MyIceServer[] iceServers) {
    ...
    public void onRemoteJoinToRoom(String socketId) {
        executor.execute(() -> {
            ...
            try {
                PeerSignalConnectionHelper.Peer mPeer = new PeerSignalConnectionHelper.Peer(socketId);
    ...
    public void onRemoteIceCandidate(String socketId, IceCandidate iceCandidate) {
        executor.execute(() -> {
            PeerSignalConnectionHelper.Peer peer = _connectionPeerDic.get(socketId);
            if (peer != null) {
    ...
    public void onReceiveOffer(String socketId, String description) {
        executor.execute(() -> {
            _role = PeerSignalConnectionHelper.Role.Receiver;
            PeerSignalConnectionHelper.Peer mPeer = _connectionPeerDic.get(socketId);
...
//            videoSource.adaptOutputFormat(mScreenWidth, mScreenHeigh, 60);
...
//            }
    ...
    private void createPeerConnections() {
        if (_connectionIdArray.size() == 0) {
            ...
            for (Object str : _connectionIdArray) {
                PeerSignalConnectionHelper.Peer peer = new PeerSignalConnectionHelper.Peer((String) str);
    ...
    private void addStreams() {
        Log.v(TAG, "为所有连接添加流");
        for (Map.Entry<String, PeerSignalConnectionHelper.Peer> entry : _connectionPeerDic.entrySet()) {
            if (_localStream == null) {
                createLocalStream();
            }
            try {
    ...
    private void createOffers() {
        for (Map.Entry<String, PeerSignalConnectionHelper.Peer> entry : _connectionPeerDic.entrySet()) {
            _role = PeerSignalConnectionHelper.Role.Caller;
            PeerSignalConnectionHelper.Peer mPeer = entry.getValue();
    ...
    private void closePeerConnection(String connectionId) {
        PeerSignalConnectionHelper.Peer mPeer = _connectionPeerDic.get(connectionId);
        if (mPeer != null) {
            mPeer.pc.close();
        }
    ...
    private class Peer implements SdpObserver, PeerConnection.Observer {
        ...
        public void onCreateSuccess(SessionDescription origSdp) {
            ...
            pc.setLocalDescription(PeerSignalConnectionHelper.Peer.this, sdp);
        }
        ...
        public void onSetSuccess() {
            ...
            if (pc.signalingState() == PeerConnection.SignalingState.HAVE_REMOTE_OFFER) {
                pc.createAnswer(PeerSignalConnectionHelper.Peer.this, offerOrAnswerConstraint());
            } else if (pc.signalingState() == PeerConnection.SignalingState.HAVE_LOCAL_OFFER) {
                //判断连接状态为本地发送offer
                if (_role == PeerSignalConnectionHelper.Role.Receiver) {
                    //接收者，发送Answer
                    _webSocket.sendAnswer(socketId, pc.getLocalDescription().description);

                } else if (_role == PeerSignalConnectionHelper.Role.Caller) {
                    //发送者,发送自己的offer
